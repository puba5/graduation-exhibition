  ...
  return (
    <>
      <ContentWrapper>
        ...
        <Textarea warningFlag={idWarningFlag} onChange={idChangeHandler} type="text" />
        {idWarningFlag && (
          <WarningMessage>아이디는 최소 6자리에서 16자리까지 입력할 수 있습니다.</WarningMessage>
        )}
      </ContentWrapper>
      <ContentWrapper>
        <InputTitle>비밀번호</InputTitle>
        <Textarea warningFlag={pwdWarningFlag} onChange={pwdChangeHandler} type="password" />
        {pwdWarningFlag && (
          <WarningMessage>비밀번호는 최소 6자리에서 16자리까지 입력할 수 있습니다.</WarningMessage>
        )}
      </ContentWrapper>
    </>
  );
